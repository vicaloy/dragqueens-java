...
public class RegistrarVictoria extends JFrame {

	private JPanel contentPane;
	private ControladorRegistrarVictoria miControlador;
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RegistrarVictoria frame = new RegistrarVictoria();
	...
	public RegistrarVictoria() {
		...
		this.miControlador = new ControladorRegistrarVictoria(this);

		setEnabled(true);
		setResizable(false);
		setTitle("Ingresar victoria");
		...
		JLabel lblParticipante = new JLabel("N\u00FAmero participante:");
		lblParticipante.setBounds(20, 48, 105, 14);
		...
		JSpinner spinnerPart = new JSpinner();
		spinnerPart.setModel(new SpinnerNumberModel(0, 0, 10, 1));
		spinnerPart.setBounds(135, 45, 48, 20);
		JFormattedTextField txtDos = ((JSpinner.NumberEditor) spinnerPart.getEditor()).getTextField();
		((NumberFormatter) txtDos.getFormatter()).setAllowsInvalid(false);
		contentPane.add(spinnerPart);
		...
		btnIngresar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				miControlador.registrarVictoria((Integer) spinnerNumero.getValue(), (Integer) spinnerPart.getValue());
			}
		});
